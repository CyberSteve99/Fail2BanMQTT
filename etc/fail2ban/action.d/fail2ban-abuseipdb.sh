...
#
#   $10 FAILURES       - >0 means ban is from local machine rather than remote mqtt. If 0 then don't add to ipabusedb.
#
# Dependencies:
...
APIKEY="$1"                 # Ignored as retrieved from file abuseipdb.apikey before use
COMMENT="$2"
IP="$3"
CATEGORIES="$4"
BANTIME="$5"
...
if [[ "${MyFails}" -eq 0 ]]; then
    log_message "INFO: (BAN) IP ${IP} has ${MyFails} failures so no action required"
    exit 0
fi
...
check_ip_in_abuseipdb() {
    ...
    oldest_report=$(jq -r '.data.reports | map(.reportedAt) | min' <<< "${body}")
...
report_ip_to_abuseipdb() {
    ...
    Score=$(jq .data.abuseConfidenceScore <<<$body)
    if [[ $total_reports > 0 ]]; then
        ReportDate=$(date -d "${oldest_report:0:4}-${oldest_report:5:2}-${oldest_report:8:2} ${oldest_report:11:2}:${oldest_report:14:2}:${oldest_report:17:2}Z" +"%Y-%m-%d %H:%M:%S")
    else
        ReportDate="Not Reported yet"
    fi
    log_message "SUCCESS: Reported ${IP} - Score: ${Score}%, Reports: ${total_reports}, Oldest Report: ${ReportDate}, Failures:${MyFails}."
    return 0
}
...
(
    is_found_local=0
    shouldBanIP=1
    . /etc/fail2ban/abuseipdb.apikey            # Get API KEY

    if check_ip_in_db $IP; then
        is_found_local=0                # Changed from 1 to 0 to force check on abuseipdb. We want additional info.
    ...
    if [[ "${shouldBanIP}" -eq 1 ]]; then
        ...
        check_ip_in_abuseipdb  # Added to check ipabusedb anyway to number of reports and oldest date from those returned.
